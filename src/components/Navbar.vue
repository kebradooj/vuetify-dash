    ...
    <v-navigation-drawer v-model="drawer" dark="" app class="blue accent-2">
      <v-layout column align-center>
        <v-flex class="mt-10 mb-8">
          <v-avatar>
            <v-icon large>fas fa-store</v-icon>
          </v-avatar>
        </v-flex>
      </v-layout>
      <v-list>
...
export default {
  data: () => ({
    ...
    links: [
      ...
      {icon: 'fas fa-credit-card', text: 'Balance', route: '/balance'},
      {icon: 'fas fa-balance-scale-right', text: 'Insurance', route: '/insurance'},
      {icon: 'fas fa-chart-line', text: 'Investment', route: '/investment'},
      {icon: 'fas fa-briefcase', text: 'Business', route: '/business'}
    ],
    slides: [
